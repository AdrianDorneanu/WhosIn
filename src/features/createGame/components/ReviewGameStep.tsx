...
function getSportMetadata(sport: string) {
	return (
		sportMetadata[sport as keyof typeof sportMetadata] ?? {
			label: sport,
			icon: faTableTennisPaddleBall,
		}
	);
...
export function ReviewGameStep({ draft }: ReviewGameStepProps) {
	const sport = getSportMetadata(draft.sport);
			...
			<View style={styles.card}>
				<View style={styles.header}>
					<View style={styles.sportIcon}>
						<FontAwesomeIcon color={colors.primary.contrast} icon={sport.icon} size={22} />
					</View>

					<View style={styles.titleContainer}>
						<Text style={styles.title}>{draft.title}</Text>
						<Text style={styles.subtitle}>{sport.label}</Text>
					</View>
				</View>
